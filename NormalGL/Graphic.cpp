...
int getRubberPoints(double &x1, double &y1, double &x2, double &y2)
{
	int int_x1, int_y1, int_x2, int_y2, temp;
	temp = getRubberPoints(int_x1, int_y1, int_x2, int_y2);
	x1 = (double)int_x1;
	y1 = (double)int_y1;
	x2 = (double)int_x2;
	y2 = (double)int_y2;
...
int getRubberPoints(Point2D pts[])
{
	PixelPoint *int_pts = new PixelPoint[g_rubberPad.pts.size()];
	getRubberPoints(int_pts);
	for (int i = 0; i < g_rubberPad.pts.size(); i++)
		pts[i] = Point2D{ (double)int_pts[i].x, (double)int_pts[i].y };
	delete int_pts;
	return 0;
}
